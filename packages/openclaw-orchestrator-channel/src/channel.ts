...
// Plain ChannelPlugin shape — no helper wrappers. The `createChatChannelPlugin`
// builder we used earlier was imported from `openclaw/plugin-sdk/channel-core`,
// which isn't present in openclaw@2026.3.x / 2026.4.x — the import silently
// fails at load time, the plugin never registers, and openclaw never dispatches
// outbound (cron messages, subagent announces, etc.) through our sendText.
export const orchestratorPlugin: ChannelPlugin<ResolvedAccount> = {
    id: "orchestrator",
    meta: {
        id: "orchestrator",
    ...
    // Control UI looks up configSchema to build a settings form. Without one
    // it renders "Unsupported type: . Use Raw mode." in the Channels panel.
    // Our three fields (enabled / pushUrl / pushSecret) are driven by the
    // orchestrator via patches.jq on provision — there's nothing useful to
    // edit here by hand, so declare an empty schema to suppress the error.
    ...
    // Without normalizeTarget+targetResolver OpenClaw's message-action-runner
    // treats any target it doesn't find in its directory cache as "Unknown
    // target" and never calls our sendText. The resolver tells core: "yes,
    // this mattermost:... string is something I own — hand it to sendText
    // as-is".
    messaging: {
    ...
    // Flat outbound adapter per ChannelOutboundAdapter schema in
    // openclaw/dist/plugin-sdk/channels/plugins/types.adapters.d.ts.
    // sendText lives at the top — not nested under "attachedResults" —
    // so it gets called for every outbound message: inbound replies,
    // cron outputs, subagent announces, BOOT welcome DMs, etc.
    outbound: {
        deliveryMode: "direct",
        sendText: async ({ cfg, to, text, accountId }) => {
            ...
            return {
                ...
                messageId: typeof result.post_id === "string" ? result.post_id : "",
            }
        },
    },
}
