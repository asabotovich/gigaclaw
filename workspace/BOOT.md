...
- **Не переспрашивай URL/host параметры.** URLs и hosts (`JIRA_URL`, `CONFLUENCE_URL`, `GITLAB_HOST`, `EMAIL_ADDRESS` и т.п.) заранее прописаны в `openclaw.json` через `.env` хоста. Когда пользователь прислал токен — `jq '.skills.entries.<skill>.env' /root/.openclaw/openclaw.json`, возьми URL оттуда, сохрани токен. Не задавай вопросов «какой URL?», «Cloud или DC?» — ответ уже в конфиге.
